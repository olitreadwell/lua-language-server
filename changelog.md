...
* `NEW` Support type inference for `dofile` and add a new `Lua.workspace.dofileRoots` setting to configure where it reads from [#3432](https://github.com/LuaLS/lua-language-server/pull/3432)
...
## 3.13.0
`2024-11-13`
* `NEW` Setting: `Lua.type.inferTableSize`: A Small Table array can be inferred
...
## 3.10.3
`2024-8-8`
* `FIX` Memory leak with `---@enum(partial)`
...
* `FIX` Now correctly evaluates the visibility of fields in a class when they are defined directly in the object. use for completion and invisible diagnostic. [#2752](https://github.com/LuaLS/lua-language-server/issues/2752)
...
## 3.6.23
`2023-7-7`
* `CHG` signature: narrow by inputted literal
...
* `FIX` description disappeared for overloaded function
* `FIX` [#1675]
...
* `CHG` [#1458] `semantic-tokens`: global variable is set to `variable.global`
  ```jsonc
...
* `NEW` completion postfix: `@function`, `@method`, `@pcall`, `@xpcall`, `@insert`, `@remove`, `@concat`, `++`, `++?`
* `CHG` `LuaDoc`:
...
* `FIX` `autoRequire` may provide duplicated options
...
* `FIX` diagnostic: may open file with wrong uri case
...
## 1.11.2
`2021-1-7`
* `FIX` [#345](https://github.com/LuaLS/lua-language-server/issues/345): not works with unexpected args
...
* `FIX` hover: tail comments may be cut
* `FIX` runtime errors
...
* `CHG` hover: `LuaDoc` also catches `--` (no need `---`)
